...
2. Padel and Storywalk (the app formerly presented as Hamming Path) are served at
   `shoebum.io/padel` and `shoebum.io/storywalk`.
...
- No feature changes to the Padel or Storywalk apps themselves.
...
- **Repo `shubhampansare/shubhampansare.github.io`** (public - GitHub Pages on
  ...
  - `/padel/index.html` - deploy artifact copied from the Padel repo
...
- **App repos**: `hamming-path` renamed to `storywalk`; then both `storywalk`
  and `padel-americano` flipped to private and their Pages sites disabled,
  only after the shoebum.io deployment is verified live.
...
- Padel: a small `deploy.sh` in the Padel repo copies `index.html` to
  `../Shoebum/padel/index.html`, then commits and pushes the site repo.
...
3. **Live projects** - three cards with real screenshots and one-liners:
   ...
   - Padel -> /padel (free Americano/Mexicano tournament app)
...
2. Rename GitHub repo `hamming-path` to `storywalk`.
3. Copy app artifacts into `/padel/` and `/storywalk/` in the site repo.
...
6. Verify live: apex, www, `/padel`, `/storywalk` all load correctly.
7. Flip `padel-americano` and `storywalk` repos to private, disable their
...
- `https://shoebum.io/padel/` and `https://shoebum.io/storywalk/` return 200
  and each app is interactive in a real browser (Playwright click-through:
  Padel reaches its setup screen, Storywalk starts and plays audio on
  interaction).
...
- Old URLs `shubhampansare.github.io/padel-americano` and `/hamming-path`
  return 404.
- `gh repo view` shows `padel-americano` and `storywalk` as private; the
